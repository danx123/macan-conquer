...
#           dengan parsing output real-time untuk menunjukkan progres sebenarnya.
# - [FITUR] Ditambahkan fungsi 'Save Log...' di menu File untuk mengekspor
#           output konsol ke file .txt.
# - [FITUR] Ditambahkan System Info Dashboard di bagian atas untuk menampilkan
#           informasi OS, CPU, RAM, dan Uptime secara real-time.
# - [DEPENDENSI] Menambahkan 'psutil' untuk mengambil data System Info.
#
# Cara Menjalankan:
# 1. Install PySide6 dan psutil: pip install pyside6 psutil
# 2. Pastikan file "toolbox.ico", "Little Registry Cleaner.exe", "Little Startup Manager.exe",
...
# --- [BARU] Worker Thread untuk Proses Latar Belakang ---
class Worker(QObject):
    """
    Worker yang berjalan di thread terpisah untuk menangani
    tugas-tugas berat agar GUI tetap responsif.
    """
    finished = Signal()
    ...
    def run(self):
        """Menjalankan fungsi target."""
        try:
            self.function(
                *self.args,
                **self.kwargs,
                # Melewatkan sinyal ke fungsi agar bisa di-emit dari sana
...
class MacanConquerApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        # --- [BARU] System Info Dashboard ---
        self.create_system_info_dashboard()
    ...
    def create_system_info_dashboard(self):
        ...
        layout.addWidget(QLabel("<b>OS:</b>"), 0, 0)
        layout.addWidget(self.os_label, 0, 1)
        layout.addWidget(QLabel("<b>CPU:</b>"), 1, 0)
        layout.addWidget(self.cpu_label, 1, 1)
        layout.addWidget(QLabel("<b>RAM:</b>"), 2, 0)
        layout.addWidget(self.ram_label, 2, 1)
        layout.addWidget(QLabel("<b>Uptime:</b>"), 3, 0)
        layout.addWidget(self.uptime_label, 3, 1)

        group_box.setLayout(layout)
        self.main_layout.addWidget(group_box)

        # Timer untuk update info secara berkala
        self.info_timer = QTimer(self)
        self.info_timer.timeout.connect(self.update_system_info)
        self.info_timer.start(2000)  # Update setiap 2 detik
        self.update_system_info()   # Panggil sekali saat start

    def update_system_info(self):
        """Mengambil dan memperbarui data di dashboard."""
        # OS Info (statis, cukup sekali)
        if self.os_label.text() == "Loading...":
             self.os_label.setText(f"{platform.system()} {platform.release()} ({platform.version()})")
        ...
        self.ram_label.setText(f"{ram.percent}% ({ram_used_gb:.2f} GB / {ram_total_gb:.2f} GB)")
        ...
        self.uptime_label.setText(f"{int(days)} hari, {int(hours)} jam, {int(minutes)} menit")
    ...
    def _save_log_to_file(self):
        """Membuka dialog untuk menyimpan isi konsol ke file .txt."""
    ...
    def _confirm_and_shutdown(self, command, message):
        reply = QMessageBox.question(self, "Konfirmasi", message,
    ...
    def _show_about_dialog(self):
        QMessageBox.about(self, "About Macan Conquer", "...")
    ...
    # --- [REFACTOR] Logika Eksekusi Proses ---
    def start_task(self, function, *args, **kwargs):
        """Template untuk memulai tugas apa pun di thread baru."""
        if self.is_running:
            QMessageBox.warning(self, "Proses Berjalan", "Satu proses sudah berjalan, harap tunggu hingga selesai.")
            return
    ...
    # --- Wrapper untuk fungsi-fungsi yang akan dijalankan di thread ---
    def run_sfc(self):
        self.start_task(self._run_command_with_progress, "sfc", ["/scannow"], "sfc")

    def run_dism(self):
        self.start_task(self._run_command_with_progress, "DISM.exe", ["/Online", "/Cleanup-image", "/Restorehealth"], "dism")
    ...
    def run_defrag(self):
        self.start_task(self._run_command_with_progress, "defrag", ["C:", "/O"])

    def clear_temp_files(self):
        self.start_task(self._clear_temp_action)

    def clear_update_cache(self):
        reply = QMessageBox.question(self, "Konfirmasi", "...", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.start_task(self._clear_update_cache_action)

    def reset_icon_cache(self):
        reply = QMessageBox.warning(self, "Konfirmasi Penting", "...", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.start_task(self._reset_icon_cache_action)

    def boot_to_safe_mode(self):
        reply = QMessageBox.warning(self, "Reboot ke Safe Mode", "...", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
    ...
    def disable_safe_mode(self):
        reply = QMessageBox.question(self, "Nonaktifkan Safe Mode", "...", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
    ...
    # --- [BARU] Fungsi-fungsi aktual yang dijalankan oleh Worker ---
    def _run_command_with_progress(self, program, args, progress_type=None, **kwargs):
        """
        Menjalankan perintah command-line dan mem-parsing outputnya
        untuk progress bar jika memungkinkan.
        """
        ...
        for line in iter(process.stdout.readline, ''):
            ...
            if clean_line:
                ...
                if progress_type == 'sfc':
                    ...
                    if match:
                        progress_signal.emit(int(match.group(1)))
                elif progress_type == 'dism':
                    match = re.search(r'\[=+>\s*\] (\d+\.\d+)%', clean_line)
                    if match:
                        progress_signal.emit(int(float(match.group(1))))
    ...
    def _clear_update_cache_action(self, **kwargs):
        ...
        try:
            ...
            output_signal.emit(f"Error mengakses {cache_dir}: {e}")
    ...
    def _reset_icon_cache_action(self, **kwargs):
        ...
        try:
            if os.path.exists(db_path):
                ...
                output_signal.emit("File IconCache.db tidak ditemukan (mungkin sudah bersih).")
        except Exception as e:
            output_signal.emit(f"Gagal menghapus IconCache.db: {e}")
